...
class BacktesterEngine(BaseEngine):
    ...
    def run_downloading(
        ...
        symbol, exchange = extract_vt_symbol(vt_symbol)  # 合约代码, 交易所
        ...
        # else:
        #     # rqdata_client.init()
        #     # data = rqdata_client.query_history_tester(req)
        #     try:
        #         data = jqdata_client.query_history_cta(req)
        ...
        #     # print("data: ", data)
        #     database_manager.save_bar_data(data)
        #     with open(f"./jqdata_{vt_symbol}_{interval}_{start}_{end}", 'wb') as f:
        #         pickle.dump(data, f)
        #     self.write_log(f"{vt_symbol}-{interval}历史数据下载完成")
        # else:
        #     self.write_log(f"保存数据失败")
