...
class CtaEngine(BaseEngine):
    ...
    def query_bar_from_jq(
        ...
        data = jqdata_client.query_history_cta(req)
        return data
    ...
    def load_bar(
        ...
        bars = self.query_bar_from_jq(symbol, exchange, interval, start, end)
    ...
    def _init_strategy(self):
        ...
        while not self.init_queue.empty():
            ...
            self.write_log(f"{strategy_name}: 策略开始执行初始化")
            ...
            self.put_strategy_event(strategy)
            self.write_log(f"{strategy_name}: 策略初始化完成")
